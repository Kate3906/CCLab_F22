...
let interactP = ["Be happy while you can", "Too bad, good things don't last forever.", "It's good to see you happy.", "Look like you are enjoying this!", "Keep up the good work!"]
...
function drawResponseToEmotion() {
  push();
  translate(width / 2, height / 2 + 150);

  fill("DarkSlateGrey");
  textSize(30);
  text(responseToEmotion, -300, -150, 600, 300);
  pop();
}
...
function getExpressions(detections) {
  if (detections.length > 0) {
    ...
    generateResponseToEmotion();
  }
}

function generateResponseToEmotion() {
  if (expression == "surprised"
    ...
    let lineChange = "\n";
    responseToEmotion = "Are you " + expression + "?" + lineChange + interactN[randomIndex];
  }
  else if (expression == "happy") {
    let randomIndex = floor(random(interactP.length));
    let lineChange = "\n";
    responseToEmotion = "You look " + expression + "." + lineChange + interactP[randomIndex];
  }
  else if (expression == "neutral") {
    responseToEmotion = "How do you feel?"
  }
}
